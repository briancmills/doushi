...
@Component({
    selector: 'jhi-verb-definition',
    templateUrl: './verb-definition.component.html'
})
export class VerbDefinitionComponent implements OnInit, OnDestroy {
    ...
    loadAll() {
        this.verbDefinitionService.query({
            page: this.page,
            size: this.itemsPerPage,
            sort: this.sort()
        }).subscribe(
            (res: HttpResponse<VerbDefinition[]>) => this.onSuccess(res.body, res.headers),
    ...
    trackId(index: number, item: VerbDefinition) {
        return item.id;
    }
    ...
    private onSuccess(data, headers) {
        ...
        for (let i = 0; i < data.length; i++) {
            this.verbDefinitions.push(data[i]);
        }
    }
